...
//     inner_nodes: &Vec<Vec<FieldElement>>,
//     root: &FieldElement,
//     depth: u32,
//     tree_index: u32,
// ) -> Result<(), Box<dyn std::error::Error>> {
//     let str: FieldElement =
...
//     let decoded: (Vec<FieldElement>, Vec<Vec<FieldElement>>, FieldElement, u32) =
//         bincode::deserialize(&buf[..])?;

//     let leaves = decoded.0;
//     // .iter()
//     // .map(|x| FieldElement::from_bytes_be(x).unwrap())
//     // .collect();
...
//         .map(|x| {
//             x.iter()
//                 .map(|y| FieldElement::from_str(y.as_str()).unwrap())
//                 .collect::<Vec<FieldElement>>()
//         })
//         .collect::<Vec<Vec<FieldElement>>>();
...
//         leaf_nodes: leaves,
//         inner_nodes,
//         root: FieldElement::from_str(&decoded.2.as_str()).unwrap(),
//         depth: decoded.3,
//         shift,
//     })
// }
